...
import { IUserService, IFirestoreService, IAuthService } from './';
import { TRANSLATE_SERVICE, FIRESTORE_SERVICE, AUTH_SERVICE, } from './injection-tokens';

import { IMeeting, Meeting, IUser, User } from '../../shared/models';
    ...
    const retryPipeline =
      ...
      retryWhen(errors => errors.pipe(
        ...
        concatMap((e, i) =>
          ...
          iif(
            () => i > environment.networkRetryCount,
            // If the condition is true we throw the error (the last error)
            throwError(e),
            // Otherwise we pipe this back into our stream and delay the retry
            of(e).pipe(map(delay(environment.networkRetryDelay))
            )
          ))));
